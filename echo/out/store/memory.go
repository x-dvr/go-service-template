...
type MemoryStore struct {
	store map[string]core.Echo
}

func NewMemoryStore() *MemoryStore {
	return &MemoryStore{
		store: make(map[string]core.Echo),
	}
}

// Load implements [ports.Store].
func (m *MemoryStore) Load(ctx context.Context, from string) (*core.Echo, error) {
...
func (m *MemoryStore) Save(ctx context.Context, e core.Echo) error {
...
func (m *MemoryStore) saveInternal(ctx context.Context, e core.Echo) error {
	start := time.Now()
...
func (m *MemoryStore) loadInternal(ctx context.Context, from string) (*core.Echo, error) {
	start := time.Now()
	time.Sleep(time.Duration(rand.IntN(100) * int(time.Millisecond)))
